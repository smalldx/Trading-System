...
int OnInit()
{
   ...
   // インジケーター名の設定
   IndicatorShortName("Bespoji Magic Trigger");
...
{
   // カウントダウンタイマーを表示
   UpdateCountdownTimer(time[0]);
   
   // Radarからセットアップ状態を取得（バッファ0と1）
   double radar_buy_signal = iCustom(NULL, 0, Radar_Indicator_Name,
                                     BB_Period, BB_Deviation, BB_AppliedPrice,
                                     Lookback_Period, Break_Validity,
                                     Enable_H1_Filter, H1_MA_Period,
                                     Show_TP_SL_Lines, SL_Offset_Pips, TP_Line_Color, SL_Line_Color,
                                     ...
                                     Buy_Arrow_Color, Sell_Arrow_Color, Arrow_Size,
                                     BB_Upper_Color, BB_Middle_Color, BB_Lower_Color, BB_Line_Width,
                                     0, 1);  // Buffer 0, shift 1（直前の確定足）
   
   double radar_sell_signal = iCustom(NULL, 0, Radar_Indicator_Name,
                                      BB_Period, BB_Deviation, BB_AppliedPrice,
                                      Lookback_Period, Break_Validity,
                                      Enable_H1_Filter, H1_MA_Period,
                                      Show_TP_SL_Lines, SL_Offset_Pips, TP_Line_Color, SL_Line_Color,
                                      ...
                                      Buy_Arrow_Color, Sell_Arrow_Color, Arrow_Size,
                                      BB_Upper_Color, BB_Middle_Color, BB_Lower_Color, BB_Line_Width,
                                      1, 1);  // Buffer 1, shift 1（直前の確定足）
   
   // バッファの初期化
   ArrayInitialize(BuySignalBuffer, EMPTY_VALUE);
   ArrayInitialize(SellSignalBuffer, EMPTY_VALUE);
   
   // 足が確定した瞬間のみシグナルを表示（shift 1 = 直前の確定足）
   if(rates_total > prev_calculated || prev_calculated == 0)
   {
      // 買いシグナルチェック
      if(radar_buy_signal != EMPTY_VALUE && radar_buy_signal != 0)
      {
         BuySignalBuffer[1] = low[1] - (30 * Point);  // 矢印表示
         
         // アラート送信（重複防止）
         if(last_buy_alert_time != time[1])
         {
            SendTriggerAlert("Buy Signal", time[1]);
            last_buy_alert_time = time[1];
         }
      }
      
      // 売りシグナルチェック
      if(radar_sell_signal != EMPTY_VALUE && radar_sell_signal != 0)
      {
         SellSignalBuffer[1] = high[1] + (30 * Point);  // 矢印表示
         
         // アラート送信（重複防止）
         if(last_sell_alert_time != time[1])
         {
            SendTriggerAlert("Sell Signal", time[1]);
            last_sell_alert_time = time[1];
         }
      }
   }
...
//| カウントダウンタイマー更新                                         |
//+------------------------------------------------------------------+
void UpdateCountdownTimer(datetime current_bar_time)
{
   // 残り時間の計算
   int seconds_left = (int)(current_bar_time + PeriodSeconds() - TimeCurrent());
   ...
   // ステータス表示
   string status_text = (seconds_left <= 3) ? "【準備】間もなくエントリー可能" : "【待機】足の確定を待っています";
   DrawStatus(status_text);
}
...
void DrawStatus(string text)
{
   ...
   ObjectSetString(0, status_obj_name, OBJPROP_TEXT, text);
   ObjectSetInteger(0, status_obj_name, OBJPROP_COLOR, clrGold);
   ObjectSetInteger(0, status_obj_name, OBJPROP_FONTSIZE, 12);
   ObjectSetString(0, status_obj_name, OBJPROP_FONT, "MS Gothic");
}
